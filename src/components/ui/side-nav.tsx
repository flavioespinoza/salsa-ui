						...
						<Link href={item.path}>
							<div
								className={`group flex h-9 items-center gap-3 rounded-lg px-5 ${
									isActive
										? `${width > 0 ? 'bg-sidenav-item-active text-sidenav-item-active-foreground shadow-md' : ''}`
										: 'text-sidenav-muted hover:bg-sidenav-item-hover hover:text-primary-foreground hover:shadow-md'
								}`}
							>
								{item.icon(isActive)}
								<span
		...
		>
			...
			>
				...
				<button onClick={toggleSideNav} className="ml-auto rounded p-1 transition-all duration-200">
					<MenuIcon
						className={`h-6 w-6 cursor-pointer text-sidenav-muted transition-transform duration-100 hover:text-foreground ${
