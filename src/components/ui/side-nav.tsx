...
export const SideNav: React.FC<SideNavProps> = ({ menuItems, menuItemsFooter, logo: Logo }) => {
							...
							<div
								className={`group flex h-9 items-center gap-3 rounded-lg px-5 ${
									isActive
										...
										: 'text-muted-foreground hover:bg-secondary/60 hover:text-secondary-foreground hover:shadow-md'
								}`}
							>
								{item.icon(isActive)}
								<span
	...
	return (
		...
		>
			<header
				id="sidebar_header"
				className={`bg-muted flex h-[74px] items-center px-4 py-3 ${width > 0 ? 'shadow-md' : ''}`}
			>
				<div
